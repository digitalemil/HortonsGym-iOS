...
NSString        *dateString, *userstr, *serverstr, *server1, *server2, *servertried, *servertotry, *hrString, *stepsString, *uploadhrstr, *uploadsuccess;
...
- (void)loadSettings {
    ...
    [myData getBytes:buf length:1024*sizeof(char)];
...
- (void)signal:(int)c {
    ...
    switch(c) {
        case 0:
            if(!years200) {
                [self.elefimg setImage:[UIImage imageNamed:@"redelefant.png"]];
            }
            else {
                self.elefimg.animationImages = [NSArray arrayWithObjects:
                                                [UIImage imageNamed:@"r0.png"],
                                                [UIImage imageNamed:@"r1.png"],
                                                [UIImage imageNamed:@"r2.png"],
                                                [UIImage imageNamed:@"r3.png"], nil];
                
                self.elefimg.animationDuration = 1.0f;
                self.elefimg.animationRepeatCount = 0;
                [self.elefimg startAnimating];
            }
            break;
        case 1:
            if(!years200) {
                ...
                [self.elefimg setImage:[UIImage imageNamed:@"greenelefant.png"]];
            }
            else {
                self.elefimg.animationImages = [NSArray arrayWithObjects:
                                                [UIImage imageNamed:@"g0.png"],
                                                [UIImage imageNamed:@"g1.png"],
                                                [UIImage imageNamed:@"g2.png"],
                                                [UIImage imageNamed:@"g3.png"], nil];
        ...
        case 2:
            if(!years200) {
                [self.elefimg setImage:[UIImage imageNamed:@"darkredelefant.png"]];
            }
            else {
                self.elefimg.animationImages = [NSArray arrayWithObjects:
                                                [UIImage imageNamed:@"d0.png"],
                                                [UIImage imageNamed:@"d1.png"],
                                                [UIImage imageNamed:@"d2.png"],
                                                [UIImage imageNamed:@"d3.png"], nil];
        ...
        case 3:
            if(!years200) {
                [self.elefimg setImage:[UIImage imageNamed:@"orangeelefant.png"]];
            }
            else {
                self.elefimg.animationImages = [NSArray arrayWithObjects:
                                                [UIImage imageNamed:@"o0.png"],
                                                [UIImage imageNamed:@"o1.png"],
                                                [UIImage imageNamed:@"o2.png"],
                                                [UIImage imageNamed:@"o3.png"], nil];
        ...
        case 4:
            if(!years200) {
                [self.elefimg setImage:[UIImage imageNamed:@"lilaelefant.png"]];
            }
            else {
                self.elefimg.animationImages = [NSArray arrayWithObjects:
                                                [UIImage imageNamed:@"l0.png"],
                                                [UIImage imageNamed:@"l1.png"],
                                                [UIImage imageNamed:@"l2.png"],
                                                [UIImage imageNamed:@"l3.png"], nil];
        ...
        case 5:
            if(!years200) {
                [self.elefimg setImage:[UIImage imageNamed:@"yellowelefant.png"]];
            }
            else {
                self.elefimg.animationImages = [NSArray arrayWithObjects:
                                                [UIImage imageNamed:@"y0.png"],
                                                [UIImage imageNamed:@"y1.png"],
                                                [UIImage imageNamed:@"y2.png"],
                                                [UIImage imageNamed:@"y3.png"], nil];
...
- (void) processStepResults:(NSArray *)results {
    ...
    if (results) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [self.stepcount setText:[NSString stringWithFormat:@"%i", (int)steps]];
            
            dateString = [formatter stringFromDate:quantitySample.startDate];
            
            stepsString = [NSString stringWithFormat:@"{'steps':[{'id':'%@-%@', 'location':'%lf,%lf', 'event_timestamp':'%@', 'user':'%@', 'steps':'%i'}]}", userstr, dateString, latitude, longitude, dateString, userstr, (int)(steps)];
            [self writeStep:stepsString];
            
            servertried= nil;
            if(servertotry!= nil)
                [self sendToServer:servertotry uri:@"/data/steps" json:stepsString];
...
- (void) processHRResults:(NSArray *)results {
    ...
    if (results) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            hrString = [NSString stringWithFormat:@"{'id':'%@-%@', 'location':'%lf,%lf', 'event_timestamp':'%@', 'deviceid':'%@', 'user':'%@', 'heartrate':'%u'}", userstr, dateString, latitude, longitude, dateString, userstr, userstr, userHR];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    NSString *uri= @"data/publish", *data= hrString;
    ...
    if(servertotry== server1 && servertried== nil && server2!= nil) {
        servertried= server1;
        servertotry= server2;
        [self sendToServer:servertotry uri:uri json:data];
        return;
    }
    if(servertotry== server2 && servertried== nil && server1!= nil) {
        servertried= server2;
        servertotry= server1;
        [self sendToServer:servertotry uri:uri json:data];
        return;
        ...
        HKSampleQuery *hrquery =[[HKSampleQuery alloc] initWithSampleType:hrType predicate:predicate limit:HKObjectQueryNoLimit sortDescriptors:nil resultsHandler:^(HKSampleQuery *query, NSArray *results, NSError *error) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                for (id object in results) {
                    ...
                    NSString *js= [NSString stringWithFormat:@"{'id':'%@-%@', 'location':'%lf,%lf', 'event_timestamp':'%@', 'deviceid':'%@', 'user':'%@', 'heartrate':'%u'},\n", userstr, [formatter stringFromDate:quantitySample.startDate], 0.0, 0.0, [formatter stringFromDate:quantitySample.startDate], userstr, userstr, userHR];
                    [ret appendString:js];
                }
                [ret appendString:@"]}"];
                //   NSLog(@"Steps: %@" , ret);
